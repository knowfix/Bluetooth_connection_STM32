...
//     // Send the corrected message as plain text WITH CHUNKING
//     // Chunking prevents Bluetooth buffer overflow
//     const int CHUNK_SIZE = 16;  // 16 bytes per chunk
//     const int CHUNK_DELAY = 5;  // 5ms delay between chunks
    
//     for (int i = 0; i < DATA_LENGTH; i += CHUNK_SIZE) {
//         int chunkLen = ((i + CHUNK_SIZE) > DATA_LENGTH) ? (DATA_LENGTH - i) : CHUNK_SIZE;
        
//         // Build chunk as String
//         String chunk = "";
//         for (int j = 0; j < chunkLen; j++) {
//             chunk += (char)correctedData[i + j];
//         }
        
//         // Send chunk without newline
//         bt.send(chunk);
        
//         delay(CHUNK_DELAY);
//     }
    
//     bt.println("");  // New line after complete message
//     bt.println("========================================");
